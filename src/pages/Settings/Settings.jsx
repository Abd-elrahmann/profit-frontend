...
export default function Settings() {
  ...
  const [autoPost, setAutoPost] = useState(settingsData?.autoPost ?? false);
  useEffect(() => {
    if (settingsData !== undefined) {
      setAutoPost(settingsData.autoPost ?? false);
    }
  }, [settingsData]);
  const handleSave = () => {
    saveMutation.mutate({ autoPost });
  };
  if (isLoading) {
    return (
      <div className="relative flex h-auto min-h-screen w-full max-w-full flex-col overflow-x-hidden dark:bg-[#141e16]">
        ...
        <div className="w-full max-w-full flex-1 py-3 sm:py-4 md:py-6 px-3 sm:px-4 md:px-6 bg-[#f6f8f6] dark:bg-[#141e16] overflow-x-hidden">
          <PageSkeleton />
        </div>
      </div>
    );
