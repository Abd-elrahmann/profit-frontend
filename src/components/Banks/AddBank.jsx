...
const AddBank = ({ open, onClose, onSuccess, bank, isEditMode = false, isMobile = false, isSmallScreen }) => {
  ...
  const validateForm = () => {
    ...
    if (!formData.owner.trim()) newErrors.owner = "اسم المالك مطلوب";
    if (!formData.limit) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      notifyError(error.response?.data?.message || `حدث خطأ أثناء ${isEditMode ? "تعديل" : "إضافة"} الحساب البنكي`);
    } finally {
      setLoading(false);
    }
  };
  const handleClose = () => {
    setFormData({ name: "", accountNumber: "", IBAN: "", limit: "", owner: "" });
    setErrors({});
    onClose();
  };
  const useFullScreen = isSmallScreen ?? isMobile;
  return (
    <Dialog
      open={open}
      onClose={handleClose}
      maxWidth={useFullScreen ? "xs" : "sm"}
      fullWidth
      fullScreen={useFullScreen}
      dir="rtl"
      PaperProps={{ sx: { borderRadius: useFullScreen ? 0 : 2 } }}
    >
      <DialogTitle sx={{ pb: 1, position: useFullScreen ? "sticky" : "static", top: 0, bgcolor: "background.paper", zIndex: 1 }}>
      ...
      <DialogActions sx={{ p: 3, gap: 1, flexDirection: "row-reverse", position: useFullScreen ? "sticky" : "static", bottom: 0, bgcolor: "background.paper", borderTop: useFullScreen ? "1px solid" : "none", borderColor: "divider" }}>
        <Button onClick={handleClose} disabled={loading} variant="outlined" fullWidth={useFullScreen} sx={{ minWidth: useFullScreen ? "auto" : "100px", borderColor: "grey.400", color: "text.secondary", "&:hover": { borderColor: "grey.500", bgcolor: isDarkMode ? "rgba(255, 255, 255, 0.05)" : "grey.50" } }}>
          إلغاء
        </Button>
        <Button variant="contained" onClick={handleSubmit} disabled={loading} fullWidth={useFullScreen} sx={{ bgcolor: "primary.main", "&:hover": { bgcolor: "primary.dark" }, minWidth: useFullScreen ? "auto" : "100px" }}>
          {loading ? <CircularProgress size={24} color="inherit" /> : isEditMode ? "تعديل" : "إضافة"}
        </Button>
      </DialogActions>
    </Dialog>
  );
