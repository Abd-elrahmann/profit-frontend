            ...
            <div className="overflow-y-auto">
              {activeStep === 0 && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  {['city', 'district'].map((field) => (
                    <div key={field} className="flex flex-col gap-1.5">
                      <label className={labelBase}>{field === 'city' ? 'المدينة' : 'الحي'}</label>
                      <input name={field} value={values[field] || ''} onChange={handleChange} onBlur={handleBlur} className={inputBase} />
                      {touched[field] && errors[field] && <span className={fieldError}>{errors[field]}</span>}
                    </div>
                  ))}
                  <div className="md:col-span-3 flex items-center gap-2 pb-2 border-b border-primary/10 mt-6 mb-2">
                  ...
                  {['employer', 'salary', 'obligations'].map((field) => (
                    <div key={field} className="flex flex-col gap-1.5">
                      <label className={labelBase}>{field === 'employer' ? 'جهة عمل الكفيل' : field === 'salary' ? 'راتب الكفيل' : 'التزامات الكفيل'}</label>
                      <input name={field} type={field === 'salary' || field === 'obligations' ? 'number' : 'text'} value={values[field] || ''} onChange={handleChange} onBlur={handleBlur} className={inputBase} />
                      {touched[field] && errors[field] && <span className={fieldError}>{errors[field]}</span>}
                    </div>
                  ))}
                </div>
