...
const GeneralLedgerSearch = ({ open, onClose, onSearch }) => {
  ...
  const formik = useFormik({
    initialValues: { account: null, fromDate: null, toDate: null },
    onSubmit: (values) => {
      onSearch({
  ...
  const handleAccountChange = (event, value) => formik.setFieldValue('account', value);
  const handleInputChange = (event, value) => setSearchInput(value);
  const handleClose = () => {
    formik.resetForm();
    setSearchInput('');
    onClose();
  ...
  return (
    <Modal open={open} onClose={handleClose} aria-labelledby="general-ledger-search-title" sx={{ backdropFilter: 'blur(2px)' }}>
      <Box sx={getModalStyle(isSmallScreen)}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3, pb: 2, borderBottom: '1px solid #e0e0e0' }}>
          <Typography id="general-ledger-search-title" variant={isSmallScreen ? "h6" : "h5"} component="h2" sx={{ fontWeight: '600', color: 'primary.main' }}>
            بحث في دفتر الأستاذ
          </Typography>
          <IconButton onClick={handleClose} size="small" sx={{ color: 'text.secondary', '&:hover': { backgroundColor: 'rgba(0, 0, 0, 0.04)' } }}>
            <CloseIcon />
          </IconButton>
        </Box>
        <form onSubmit={formik.handleSubmit}>
          <Stack spacing={isSmallScreen ? 2 : 3}>
              ...
              renderOption={(props, option) => (
                <Box component="li" {...props} sx={{ py: 1 }}>
                  <Box sx={{ display: 'flex', flexDirection: 'column', width: '100%' }}>
                    <Typography variant="body1" fontWeight="500" noWrap>{option.code} - {option.name}</Typography>
                    <Typography variant="caption" color="text.secondary">{getAccountTypeArabic(option.type)}</Typography>
                  </Box>
                </Box>
              )}
              renderInput={(params) => (
                <TextField
                  ...
                  helperText={formik.touched.account && !formik.values.account ? 'الحساب مطلوب' : ''}
                  InputProps={{
                    ...params.InputProps,
                    endAdornment: <>{isLoading ? <CircularProgress size={20} /> : null}{params.InputProps.endAdornment}</>,
                  }}
            ...
            />
            <Stack direction={isSmallScreen ? "column" : "row"} gap={2} sx={{ mt: 2, flexDirection: "row-reverse", justifyContent: "space-between" }}>
              <Button variant="outlined" fullWidth onClick={handleReset} startIcon={<RestartAlt sx={{ marginLeft: "10px" }} />} size={isSmallScreen ? "medium" : "large"}
                sx={{ color: 'text.secondary', borderColor: 'text.secondary', '&:hover': { borderColor: 'text.primary', bgcolor: 'action.hover' }, order: isSmallScreen ? 2 : 1 }}>
                إعادة تعيين
              </Button>
              <Button variant="contained" fullWidth type="submit" disabled={!formik.values.account} startIcon={<Search sx={{ marginLeft: "10px" }} />} size={isSmallScreen ? "medium" : "large"}
                sx={{ bgcolor: 'primary.main', '&:hover': { bgcolor: 'primary.dark' }, '&:disabled': { bgcolor: 'action.disabled', color: 'text.disabled' }, order: isSmallScreen ? 1 : 2 }}>
                بحث
              </Button>
            </Stack>
            {formik.values.account && (
              <Box sx={{ p: 2, borderRadius: 2, bgcolor: 'primary.50', border: '1px solid', borderColor: 'primary.100', mt: 1 }}>
                <Typography variant="subtitle2" fontWeight="bold" color="primary.main" gutterBottom>الحساب المحدد:</Typography>
                <Typography variant="body2" fontWeight="medium">{formik.values.account.code} - {formik.values.account.name}</Typography>
                <Typography variant="caption" color="text.secondary">النوع: {getAccountTypeArabic(formik.values.account.type)}</Typography>
              </Box>
            )}
          </Stack>
        </form>
